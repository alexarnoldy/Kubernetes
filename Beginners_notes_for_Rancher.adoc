...
NOTE: This is referenced from the MetalLB portion of this doc: https://github.com/alexarnoldy/CaaSP/blob/master/Nutanix/Nginx_Ingress_Controller_and_MetalLB.adoc as it seems easier to deploy it via manifests than through the UI.
...
* Set at least the default IP range and, optionally, the reserved IP range that will not be auto-assigned (Note that IP ranges can also be defined by CIDR notation. Adjust this and the configmap file as needed.)
----
...
NOTE: This guide assumes Istio was installed when the RKE cluster was instantiated. If this is not the case, skip this section and install Istio as part of the Kubeflow installation. Other ingress options are beyond the scope of this guide.
