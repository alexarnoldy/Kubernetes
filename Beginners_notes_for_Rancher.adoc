...
** Verify the resources were created correctly (and that the service/nginx has an EXTERNAL-IP): `kubectl get pod,svc,pvc -n kubeflow`
...
* Use the following command to monitor the kubeflow deployment: `watch 'kubectl get pods -A | egrep -v "Completed|Running"'`
